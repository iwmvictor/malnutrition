...
export function AdminDashboard({ adminLevel }: AdminDashboardProps) {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      case 'high': return 'text-orange-700 bg-orange-100 dark:bg-orange-900 dark:text-orange-200';
      case 'medium': return 'text-yellow-700 bg-yellow-100 dark:bg-yellow-900 dark:text-yellow-200';
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'investigating': return 'text-yellow-700 bg-yellow-100 dark:bg-yellow-900 dark:text-yellow-200';
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                {adminLevel.charAt(0).toUpperCase() + adminLevel.slice(1)} Admin Dashboard
              </h1>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={malnutritionTrendData}>
                    ...
                    <Area type="monotone" dataKey="moderate" stackId="1" stroke="#F59E0B" fill="#F59E0B" fillOpacity={0.6} />
                    <Area type="monotone" dataKey="severe" stackId="1" stroke="#EF4444" fill="#EF4444" fillOpacity={0.6} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                    {criticalAlerts.filter(alert => alert.status === 'pending').length} Pending
                  </span>
                </div>
                <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                            {alert.area} • {alert.affectedChildren} children affected
                          </p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={interventionImpactData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="interventions" stroke="#3B82F6" strokeWidth={2} name="Interventions" />
                  <Line type="monotone" dataKey="improved" stroke="#10B981" strokeWidth={2} name="Improved Cases" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            {recentActions.map((action) => (
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  action.type === 'assignment' ? 'bg-blue-100 text-blue-600' :
                  action.type === 'intervention' ? 'bg-green-100 text-green-600' :
                  'bg-orange-100 text-orange-600'
                }`}>
                  {action.type === 'assignment' ? <Users className="w-5 h-5" /> :
                   action.type === 'intervention' ? <Activity className="w-5 h-5" /> :
